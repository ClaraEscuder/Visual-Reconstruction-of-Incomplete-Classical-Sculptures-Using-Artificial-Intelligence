#extension del v6: ademas del PNG binario, escribe un cond.npz por imagen con tres
#campos consolidados (part_id, U, V) que combinan lo que se observa en el DensePose
#del cuerpo VISIBLE con lo que se proyecta para el miembro FALTANTE.
#
#esto sirve como condicionamiento estructural para LaMa-v7: el primer convolucional
#de la red leera 7 canales (RGB + mask + part_id + U + V) en vez de 4. el part_id en
#la zona enmascarada le dice al modelo "aqui deberia haber antebrazo izquierdo", y las
#UV le dan una idea de orientacion sobre el mesh SMPL.
#
#toda la logica de proyeccion (simetria, fallback continuo extremidades, fallback PCA
#cabeza) es identica al v6: lo unico que cambia es que ahora cada region proyectada
#se guarda con su nombre para luego pintar el cond.npz, y se anade un postproceso que
#construye los tres campos del cond.
#
#paths e inputs son los mismos que v6, salvo que tambien se escribe el cond.npz.
...
#part_id canonico que se asigna a la zona proyectada de cada region. usamos la
#primera de las dos caras DensePose porque para el conditioning solo importa
#la identidad anatomica de la region, no la cara frontal/posterior del mesh
...
def cargar_predicciones(npz_path):
    ...
    #U y V son opcionales para la generacion de mascara pero imprescindibles para el cond
...
#proyeccion continua para brazos / piernas (logica v6):
def es_brazo(region):
...
#proyeccion fallback PCA (cabeza):
...
def sintetizar_uv_en_region(mascara_region: np.ndarray):
    ...
    (float32, mismo shape)  --> son gradientes lineales a lo largo del eje principal
    de la region (U: 0 en el extremo proximal, 1 en el distal) y del eje perpendicular
    (V: 0 a 1 cruzando el ancho de la region).

    NO ES una parametrizacion SMPL real pero da al modelo una nocion consistente de
    "donde dentro de la region estoy", que es lo que necesita para usar el part_id
    como conditioning. los pixels fuera de la mascara quedan a 0
    """
    ...
    #normalizamos a [0, 1] sobre el rango observado dentro de la mascara
...
def generar_mascara_y_cond(npz_path):
    ...
    #recolectamos las proyecciones por region para poder pintar el cond despues
    ...
    #construccion del cond:
    #  I_pred parte del body_parts original (cuerpo visible)
    #  para cada region proyectada, los pixels que (a) estan dentro de su mascara proyectada
    #  y (b) no son cuerpo visible se etiquetan con el part_id canonico de la region
    #  U_pred y V_pred parten del U/V original; en las zonas proyectadas se sintetizan con
    #  un gradiente lineal a lo largo del eje principal de la region proyectada.
...
def main():
    ...
    log_path = Path("/home/pfc/cescuder/tfg/logs/compute_mask_densepose_v6_1.log")
    ...
    for npz_path in tqdm(npz_files, desc="generating masks v6.1"):
        stem = npz_path.stem
        mask_path = DIR_SALIDA / f"{stem}_mask.png"
        cond_path = DIR_SALIDA / f"{stem}_cond.npz"

        #si ambos existen ya, saltamos; si solo el PNG existe (de una corrida v6 previa),
        #regeneramos para producir tambien el cond
        if mask_path.exists() and cond_path.exists():
            procesadas += 1
            continue

        try:
            mascara, cond, status, info = generar_mascara_y_cond(npz_path)
    ...
    log.info("mask + cond generation v6.1 completed")
    log.info(f"  procesadas: {procesadas}")
    log.info(f"  cond escritos: {cond_escritos}")
