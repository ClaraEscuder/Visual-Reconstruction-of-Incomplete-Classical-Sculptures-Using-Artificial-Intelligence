#genera mascaras y cond.npz para los broken_body con siluetas anatomicas
#visibles, optimizadas para la convencion artistica de escultura clasica:
#
#  - dilatacion minima (3 px) para preservar la silueta exacta
#  - taper agresivo (factor 0.65) reflejando el afilamiento exagerado de
#    las extremidades en escultura clasica idealizada
#  - proyeccion por segmento independiente, con cada segmento conectado al
#    extremo distal del anterior (upper_arm -> lower_arm -> hand)
#  - mano y pie como ovalo PERPENDICULAR al eje del brazo / pierna,
#    reflejando que la muneca rompe el eje del antebrazo
#  - fallback a proyeccion combinada (estilo v7) si la per-segmento falla
#
#para cada imagen se escribe:
#  - mascara.png binaria con la region a inpaintar
#  - cond.npz con tres canales (I_pred, U_pred, V_pred) para conditioning
#    DensePose de LaMa-v7 / v8
...
DIR_SALIDA = Path("/home/pfc/cescuder/tfg/masks/broken_body_v8")
...
COBERTURA_ESPERADA = {
    "torso": 0.180, "right_hand": 0.018, "left_hand": 0.018,
    "left_foot": 0.020, "right_foot":  0.020, "upper_leg_R": 0.060,
    "upper_leg_L": 0.060, "lower_leg_R": 0.050, "lower_leg_L": 0.050,
    "upper_arm_L": 0.040, "upper_arm_R": 0.040, "lower_arm_L": 0.040,
    "lower_arm_R": 0.040, "head": 0.060,
}

PORCENTAJE_VERTICAL = {
    "head": 0.13, "torso": 0.30,
    "upper_leg_L": 0.24, "upper_leg_R": 0.24,
    "lower_leg_L": 0.22, "lower_leg_R": 0.22,
    "upper_arm_L": 0.16, "upper_arm_R": 0.16,
    "lower_arm_L": 0.16, "lower_arm_R": 0.16,
    "left_hand": 0.10, "right_hand": 0.10,
    "left_foot": 0.07, "right_foot": 0.07,
}
...
SIMETRICA = {
    ...
    "upper_leg_R": "upper_leg_L", "upper_leg_L": "upper_leg_R",
    "lower_leg_R": "lower_leg_L", "lower_leg_L": "lower_leg_R",
...
PADRE = {
    ...
    "lower_leg_L": "upper_leg_L", "lower_leg_R": "upper_leg_R",
    "upper_leg_L": "torso", "upper_leg_R": "torso",
    "left_hand": "lower_arm_L", "right_hand": "lower_arm_R",
...
ABUELO = {
    "left_foot":   "upper_leg_L", "right_foot":  "upper_leg_R",
    "lower_leg_L": "torso",       "lower_leg_R": "torso",
    "left_hand":   "upper_arm_L", "right_hand":  "upper_arm_R",
    "lower_arm_L": "torso",       "lower_arm_R": "torso",
    "head": None,
}


DIMENSIONES = {
    "left_foot": (0.10, 0.13), "right_foot": (0.10, 0.13),
    "left_hand": (0.13, 0.10), "right_hand": (0.13, 0.10),
    "lower_arm_L": (0.22, 0.10), "lower_arm_R": (0.22, 0.10),
    "upper_arm_L": (0.20, 0.11), "upper_arm_R": (0.20, 0.11),
    "lower_leg_L": (0.28, 0.12), "lower_leg_R": (0.28, 0.12),
    "upper_leg_L": (0.30, 0.14), "upper_leg_R": (0.30, 0.14),
    "head": (0.18, 0.14),
}


#factor de afilamiento distal mas agresivo: 0.65 refleja la idealizacion
#anatomica caracteristica de la escultura clasica griega y romana, donde el
#tono muscular y la forma se exageran respecto a las proporciones medias de
#una poblacion contemporanea (estadisticas COCO)
ANCHO_DISTAL_FACTOR = 0.65

#dilatacion minima para preservar la forma anatomica de la silueta. valores
#altos (como en v6/v7) borraban las transiciones de ancho entre segmentos y
#convertian las siluetas en blobs casi rectangulares
DILATAR_PX_MIN = 3
DILATAR_PX_FACTOR = 200.0
MIN_PIX_PADRE = 30
...
#silueta anatomica orientada:
def construir_silueta_orientada(punto_anclaje, eje, largo_px, ancho_px, shape, forma="ahusada", ancho_distal_factor=ANCHO_DISTAL_FACTOR, solape_interno=0.0):
    #forma "ahusada":  ancho varia linealmente de ancho_px (proximal) a
    #                  ancho_px*ancho_distal_factor (distal). brazos, piernas.
    #forma "eliptica": elipse simetrica, mas ancha en el medio. cabeza.
    #forma "ovalada":  elipse compacta de baja excentricidad. mano, pie.
...
#proyeccion para extremidades: per-segmento con conexion en cadena:
def es_brazo(region):
...
def proyectar_extremidad(region, mascaras_por_region, alto_referencia, coberturas):
    #per-segmento con conexion en cadena. cada region se proyecta con sus
    #propias dimensiones ANCHO y LARGO de DIMENSIONES, anclada en el extremo
    #distal del segmento anterior. mano y pie usan forma ovalada con eje
    #perpendicular al brazo / pierna, reflejando la articulacion.
    ...
    if idx_ultima_completa == -1:
        ...
        anchor_segmento_proximal = np.array([anc_y, anc_x])
        eje = eje_proximal
        ...
        if es_brazo(region):
            ...
            if eje_ref[0] < 0:
                eje_ref = -eje_ref
        anchor_segmento_proximal = punto_extremo_distal(m_ref, eje_ref)
        eje = eje_ref
        info_metodo = f"desde_{cadena[idx_ultima_completa]}"

    #avanzamos en cadena desde idx_ultima_completa+1 hasta idx_region (inclusive)
    #generando una silueta por cada segmento, cada una anclada al extremo distal
    #del anterior
    union_mascaras = np.zeros_like(m_torso, dtype=bool)
    anchor_actual = anchor_segmento_proximal
    eje_actual = eje
    solape_px = float(SOLAPE_INTERNO_PCT * alto_referencia)

    regiones_a_proyectar = cadena[idx_ultima_completa + 1 : idx_region + 1]

    for r_seg in regiones_a_proyectar:
        largo_pct, ancho_pct = DIMENSIONES[r_seg]
        largo_px = float(largo_pct * alto_referencia)
        ancho_px = float(ancho_pct * alto_referencia)

        if es_mano_o_pie(r_seg):
            #la articulacion muneca/tobillo rompe la direccion del antebrazo /
            #pantorrilla. la mano y el pie se modelan como ovalo perpendicular
            #al eje del segmento anterior
            eje_perp = np.array([-eje_actual[1], eje_actual[0]])
            #orientar el perpendicular consistente: para brazos derechos el
            #perpendicular apunta a la izquierda visual, para izquierdos a la
            #derecha (refleja anatomia natural de manos colgantes)
            mascara_seg = construir_silueta_orientada(
                anchor_actual, eje_perp, ancho_px * 1.4, largo_px * 0.9,
                m_torso.shape, forma="ovalada", solape_interno=solape_px,
            )
        else:
            mascara_seg = construir_silueta_orientada(
                anchor_actual, eje_actual, largo_px, ancho_px,
                m_torso.shape, forma="ahusada",
                ancho_distal_factor=ANCHO_DISTAL_FACTOR,
                solape_interno=solape_px,
            )

        union_mascaras |= mascara_seg
        #avanzar el anchor al extremo distal de este segmento, manteniendo el
        #eje actual para el siguiente (excepto si era mano/pie, en cuyo caso
        #no continua nada distal)
        anchor_actual = anchor_actual + eje_actual * largo_px

    if union_mascaras.sum() == 0:
        return None, {"error": "proyeccion_vacia"}

    info = {
        "metodo": info_metodo,
        "lado": lado,
        "regiones_cubiertas": regiones_a_proyectar,
        "anchor_inicial": anchor_segmento_proximal,
        "eje": eje,
        "forma": "per_segmento_ahusada+ovalada_perp",
    }
    return union_mascaras, info

#proyeccion fallback para cabeza:
def proyectar_fallback_anatomico(region, mascaras_por_region, alto_referencia):
    ...
    mascara_proy = construir_silueta_orientada(punto_anclaje, eje, largo_px, ancho_px, m_padre.shape, forma=forma, solape_interno=solape_px,)
...
def generar_mascara_y_cond(npz_path):
    ...
    for region, obs, esp in mutiladas:
        ...
        if m_proy is not None:
            ...
            info["proyecciones"][region] = {"metodo": "simetria"}
            continue
        ...
        info["proyecciones"][region] = {"metodo": "saltado"}
...
def main():
    ...
    log_path = Path("/home/pfc/cescuder/tfg/logs/compute_mask_densepose_v8.log")
    ...
    for npz_path in tqdm(npz_files, desc="masks v8"):
        try:
            stem = npz_path.stem
    ...
    log.info("COMPUTE MASK V8 COMPLETED")
