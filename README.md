...
### The mask generator is the most fragile component

Several of the failure modes observed in the inpainters trace back to the mask generator rather than to the generators themselves. The current implementation is purely geometric (DensePose part labels + UV coordinates + contralateral mirroring or PCA extrapolation) and has four recurrent issues that motivate the most concrete future-work directions: (i) it assumes a single SMPL instance per image, so it fails systematically on multi-figure compositions (mythological groups, paired figures) where DensePose mixes the parts of the different bodies; (ii) on unusual poses (e.g. crouching figures) the pose statistics of the generator do not match the actual anatomy and the mask lands in the wrong place, which is the root cause of SD's two failure modes (wing on a misplaced mask, furniture on a more severely misplaced one); (iii) small distal parts adjacent to a surviving stump (missing hands, feet, forearm endings) tend to be missed because DensePose extends the part label across the stump; (iv) stray patches occasionally appear on drapery or pedestal due to spurious DensePose activations. The manual review checkpoint of Phase 1 partially mitigates the multi-body problem, but a learned mask predictor (trained on a small manually-annotated sculpture set) would likely outperform the current geometric projection on all four cases. This is listed as the third future-work direction in the project memoir.

### Example: per-sculpture card (kouros from Sizilien, marble)

![Kouros example](figures/grid_2009-03-22_03-29_Sizilien_686_Agrigent_Parco_Valle_dei_Templi_Agrigento_Museo_Archaeologico-2.png)

A frontal-view kouros with a missing right arm. The anatomical mask (red overlay) also covers a small patch on the lower leg, a residual mask-generator error caused by the figure's pose; on that patch, both MAT and SD generate a thin, rod-like structure rather than any body-like content, correctly reflecting that no human anatomy is expected there despite the patch.

- **LaMa baseline**: fills the masked region with a smooth, low-contrast colour blur with no texture or sculpted structure.
- **LaMa with pipeline**: leaves the masked region essentially unmodified.
- **MAT baseline**: generates a completely different, dark, amorphous shape that spills well beyond the intended patch, distorting most of the image.
- **MAT with pipeline**: fills the mask with marble-toned content (coherent colour, texture and shading) that follows the silhouette of the patch itself; it adapts the size to anatomically plausible proportions but does not depart from the mask shape.
- **SD baseline**: makes only minimal changes anchored right at the edge of the surviving body, without reconstructing across the patch.
- **SD with ControlNet**: produces the most realistic reconstruction, with its own shape (not bound to the mask silhouette, unlike MAT), convincing marble texture and shading.
- The reconstructed foot is imperfect in the toe region in every with-pipeline configuration.
- The missing hand is not covered by any mask at all: this is a separate mask-generator failure where DensePose extends the surviving forearm's part label across the stump, so the missing hand is never flagged as missing and no model attempts to reconstruct it.
