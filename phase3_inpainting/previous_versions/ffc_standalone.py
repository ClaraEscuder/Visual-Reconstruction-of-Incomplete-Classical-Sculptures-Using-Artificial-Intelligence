#FFCResNetGenerator autocontenido. extraido del repo advimman/lama
#(saicinpainting/training/modules/) y reorganizado para que no dependa del paquete
#saicinpainting. solo necesita torch + kornia (para LearnableSpatialTransformWrapper).
#
#se usa para cargar los pesos de big-lama (best.ckpt) sin tener que arrastrar todas
#las dependencias de saicinpainting (albumentations 0.5, pytorch_lightning 1.6,
#pkg_resources, etc) que estan rotas en envs modernos.
#
#parametros que big-lama (b18_ffc075) espera para reconstruir el generator:
...
#utilidades (originalmente en saicinpainting.utils y modules/base.py):
...
#squeeze and excitation (originalmente en modules/squeeze_excitation.py):
...
#spatial transform wrapper (originalmente en modules/spatial_transform.py)
#solo se usa si spatial_transform_layers no es None. big-lama no lo usa, asi que
#esta clase queda como placeholder por compatibilidad con la firma original:
...
class LearnableSpatialTransformWrapper(nn.Module):
    ...
    def forward(self, x):
        if torch.is_tensor(x):
            return self._inverse(self.impl(self._transform(x)), x)
        elif isinstance(x, tuple):
            x_t = tuple(self._transform(e) for e in x)
            y_t = self.impl(x_t)
            return tuple(self._inverse(e, o) for e, o in zip(y_t, x))
        else:
    ...
    def _transform(self, x):
        if self._rotate is None:
            return x
        h, w = x.shape[2:]
        pad_h, pad_w = int(h * self.pad_coef), int(w * self.pad_coef)
        x_p = F.pad(x, [pad_w, pad_w, pad_h, pad_h], mode="reflect")
        return self._rotate(x_p, angle=self.angle.to(x_p))

    def _inverse(self, y_p, orig_x):
        if self._rotate is None:
            return y_p
        h, w = orig_x.shape[2:]
        pad_h, pad_w = int(h * self.pad_coef), int(w * self.pad_coef)
        y_p = self._rotate(y_p, angle=-self.angle.to(y_p))
        yh, yw = y_p.shape[2:]
        return y_p[:, :, pad_h:yh - pad_h, pad_w:yw - pad_w]


#fast fourier convolution (originalmente en modules/ffc.py):
...
class FFC(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, ratio_gin, ratio_gout,
        ...
        self.convl2l = module(in_cl, out_cl, kernel_size, stride, padding, dilation,
                              groups, bias, padding_mode=padding_type)
...
class FFCResNetGenerator(nn.Module):
    def __init__(self, input_nc, output_nc, ngf=64, n_downsampling=3, n_blocks=9,
                 ...
                 spatial_transform_layers=None, spatial_transform_kwargs=None,
        ...
        spatial_transform_kwargs = spatial_transform_kwargs or {}
...
#parametros del generator que usa big-lama (b18_ffc075). definidos aqui para que cualquier script pueda importarlos sin tener que leer config.yaml:

BIG_LAMA_GENERATOR_KWARGS = dict(
    input_nc=4,
    output_nc=3,
    ngf=64,
    n_downsampling=3,
...
def cargar_pesos_big_lama(generator: nn.Module, ckpt_path: str, device: str = "cuda") -> None:
    """
    Carga los pesos de big-lama (best.ckpt) en un FFCResNetGenerator instanciado
    con BIG_LAMA_GENERATOR_KWARGS. el checkpoint es de pytorch lightning, asi que
    las claves vienen con prefijo "generator.model.X.Y.Z"; aqui las filtramos y
    les quitamos el prefijo antes de cargarlas en el modelo.
    """
