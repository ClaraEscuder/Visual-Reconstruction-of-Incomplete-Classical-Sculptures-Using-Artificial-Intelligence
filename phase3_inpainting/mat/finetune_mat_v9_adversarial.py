"""
Fine-tune adversarial INTENSIFICADO sobre MAT v8 (genera v9) — FASE 2 del
schedule adversarial de dos fases.

Warm-start desde MAT v8 (que ya tuvo 8 epochs de fase suave con peso_adv=0.01)
para aplicar 6 epochs adicionales con peso_adv=0.5. Schedule simetrico al de
LaMa (v8 + v9), garantizando que la comparacion entre arquitecturas en la
memoria sea metodologicamente equivalente.

En la memoria se reporta UNA UNICA variante "adversarial" por modelo (la v9),
sin desglosar v8/v9. La exploracion de otros pesos adversariales queda como
future work.
...
OUTPUT:
    - ~/tfg/MAT/checkpoints/best_finetuned_mat_v9.pt
    - ~/tfg/MAT/checkpoints/last_finetuned_mat_v9.pt
    - ~/tfg/logs/finetune_mat_v9.log
"""
...
CKPT_V8 = DIR_CKPT / "best_finetuned_mat_v8.pt"
CKPT_BEST = DIR_CKPT / "best_finetuned_mat_v9.pt"
CKPT_LAST = DIR_CKPT / "last_finetuned_mat_v9.pt"


TAMANO_IMG = 512
BATCH_SIZE = 2
EPOCHS = 6 #--> 6 epochs adicionales sobre v8 (simetrico a LaMa v9)
LR_G = 5e-6
LR_D = 2e-5
VAL_SPLIT = 0.10
NUM_WORKERS = 2
...
logging.basicConfig(
    ...
    handlers=[logging.StreamHandler(),
              logging.FileHandler(BASE / "logs" / "finetune_mat_v9.log", encoding="utf-8")])
log = logging.getLogger(__name__)


def cargar_mat_v9(device):
    """Construye MAT expandido a 7 canales y carga los pesos v8 como warm start."""
    if not CKPT_V8.exists():
        raise FileNotFoundError(f"v8 checkpoint not found at {CKPT_V8}. Run finetune_mat_v8_adversarial.py first.")
    ...
    if CKPT_LAST.exists():
        log.info(f"resuming v9 from {CKPT_LAST}")
        ...
        log.info(f"warm-start desde v8 (fase suave): {CKPT_V8}")
        state = torch.load(str(CKPT_V8), map_location=device, weights_only=False)
        G.load_state_dict(state["generator"], strict=False)
        log.info("pesos v8 cargados, fase fuerte adversarial (peso_adv=0.5) desde epoch 0")
        start_epoch = 0
        best_val = float("inf")
        epochs_sin_mejora = 0
...
def main():
    ...
    G, start_epoch, best_val, epochs_sin_mejora = cargar_mat_v9(device)
    G.train()
    ...
    log.info(f"FINE-TUNE v9 MAT (HARD ADV, peso_adv=0.5) COMPLETED - best val loss: {best_val:.4f}")
