"""
Fine-tune adversarial intensificado sobre LaMa v8 (genera lo que llamamos v9).

Punto de partida: el checkpoint v8 que ya tenia adversarial con peso 0.01.
Subimos el peso adversarial a 0.5 (50x mas fuerte) para forzar al generador a
producir contenido con textura/sombras de alta frecuencia, rompiendo la
tendencia a la solucion lisa observada en v8. Pocas epocas porque es polishing.

Loss G = L1 + lambda_perc * perceptual + 0.5 * adversarial
...
INPUT:
    ...
    - pesos v8:   ~/tfg/lama_repo/big-lama/models/best_finetuned_v8.ckpt

OUTPUT:
    - ~/tfg/lama_repo/big-lama/models/best_finetuned_v9.ckpt
    - ~/tfg/lama_repo/big-lama/models/last_finetuned_v9.ckpt
    - ~/tfg/logs/finetune_lama_v9.log
"""
...
CKPT_V8 = DIR_BIG_LAMA / "models" / "best_finetuned_v8.ckpt"
CKPT_FINETUNED = DIR_BIG_LAMA / "models" / "best_finetuned_v9.ckpt"
CKPT_LAST = DIR_BIG_LAMA / "models" / "last_finetuned_v9.ckpt"
...
logging.basicConfig(
    ...
    handlers=[logging.StreamHandler(),
              logging.FileHandler(BASE / "logs" / "finetune_lama_v9.log", encoding="utf-8")])
...
def cargar_generator_v9(device):
    """Construye el generator 7-canal y carga los pesos v8 como warm start
    para continuar entrenamiento con peso_adv intensificado."""
    if not CKPT_LAMA.exists():
        raise FileNotFoundError(f"big-lama checkpoint not found at {CKPT_LAMA}")
    if not CKPT_V8.exists():
        raise FileNotFoundError(f"v8 checkpoint not found at {CKPT_V8}. Run v8 first.")
    ...
    if CKPT_LAST.exists():
        ...
        log.info(f"warm-start desde v8: {CKPT_V8}")
        state = torch.load(str(CKPT_V8), map_location=device, weights_only=False)
        gen_sd = {k[len("generator."):]: v for k, v in state["state_dict"].items() if k.startswith("generator.")}
        generator.load_state_dict(gen_sd, strict=False)
        log.info("  pesos v8 cargados, polishing con adversarial fuerte desde epoch 1")
        start_epoch = 1
        best_val = float("inf")
...
def main():
    ...
    generator, start_epoch, best_val = cargar_generator_v9(device)
