"""
Inferencia de LaMa v8 adversarial sobre broken_body bg-removed.

Carga los pesos v8 (resultado del finetune adversarial sobre v7 NOBG) y procesa
las imagenes de broken_body sin fondo usando el conditioning DensePose en cond.npz.

INPUT:
    - imagenes:        ~/tfg/background_removed/broken_body/
    - mascaras y cond: ~/tfg/masks/broken_body/{stem}_mask.png + {stem}_cond.npz
    - pesos:           ~/tfg/lama_repo/big-lama/models/best_finetuned_v8.ckpt

OUTPUT:
    - reconstrucciones: ~/tfg/inpainting_results/lama_v8_adversarial/
"""
...
DIR_SALIDA = BASE / "inpainting_results" / "lama_v8_adversarial_v8masks"
...
logging.basicConfig(
    ...
    handlers=[logging.StreamHandler(),
              logging.FileHandler(BASE / "logs" / "lama_v8_adversarial_v8masks.log", encoding="utf-8")])
...
def main():
    ...
    for img_path in tqdm(imagenes, desc="LaMa v8 adversarial"):
        try:
            salida_path = DIR_SALIDA / (img_path.stem + "_lamav8.png")
    ...
    log.info("LaMa v8 adversarial COMPLETED")
